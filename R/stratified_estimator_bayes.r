...
#'
#' @param groupvec If `ext = "RGN"` `groupvec = seq(groupnames)`
#'                 If `ext = "bot` a vector the same length as the number of populations, each element is the group number for the corresponding population.
...
#' @param ext Used to indicate if "RGN" (group estimates) or "BOT" (population estimates) files are to be read in. Defaults to "RGN".
...
#' @param xlsx Logical; if TRUE, the output will be written to an Excel file.
...
#'
#' @export
stratified_estimator_bayes <- function(groupvec, groupnames, maindir, mixvec, catchvec, CVvec = rep(0,length(catchvec)), newname, priorname = "", ext = "RGN", nchains = 5, burn = 0.5, alpha = 0.1, threshold = 5e-7, xlsx = FALSE, PosteriorOutput = FALSE){
  
  nstrata <- length(mixvec)
  
  if(nstrata==1){
    
    stop("This function is for multiple strata!!!")
    
  }
  
  if(length(mixvec) != length(catchvec)) {
    
    stop("mixvec and catchvec must be the same length")
    
  }
  
  chains <- paste("Chain", 1:nchains, sep = "")
  
  files <- sapply(mixvec,function(mix){
    
    sapply(chains,function(chain){
      
      paste(maindir, "\\", mix, "\\", mix, priorname, chain, ext, ".", ext, sep = "")
  ...
  for(chain in chains){
    ...
    H0[[chain]] <- sapply(seq_along(mixvec), function(mix){
    ...
    WghtAve0[[chain]] <- Reduce("+", lapply(seq_along(mixvec), function(mix){
      
      H0[[chain]][[mix]]*output[[chain]][[mix]]
      
    }))/Reduce("+", lapply(seq_along(mixvec), function(mix){
    ...
    WghtAve[[chain]] <- coda::as.mcmc(WghtAve[[chain]])
    
  }
  
  WghtAve <- coda::as.mcmc.list(WghtAve)
  ...
  summary[groupnames,"P=0"] <- apply(OutputWghtAve, 2, function(clm){
    
    sum(clm<threshold)/n
    
  })
  
  summary[groupnames, "GR"] <- coda::gelman.diag(WghtAve, multivariate = FALSE, transform = TRUE)[[1]][ , 1]
  
  if(xlsx){
    
    xlsx::write.xlsx(summary, file = paste(maindir, "\\", newname, ".xlsx", sep = ""))
