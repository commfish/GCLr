...
#' 
#' This function takes the unmodified output from [rubias::self_assign], plots the scaled likelihood for each baseline sample (y-axis) by reporting group or population in a box plot, and plots the true positive rate (y-axis) by precision (x-axis)
...
#' 
#' @param by_group logical; whether you want the boxplot by reporting group (TRUE) or population (FALSE); default = TRUE
#' 
#' @param min_tpr the true positive rate cutoff for adding a horizontal dashed line to the precision plot; default = 0.80
#' 
#' @param min_pre the precision cutoff for adding a vertical dashed line to the precision plot; default = 0.95
#' 
...
#'              \itemize{
#'                \item{\emph{facet}: }{An iteractive plot of TPR vs Precision with all reporting groups (i.e., faceted plot}
#'                \item{\emph{paired}: }{Interative plots for each reporting group containing a box plot of assignement likelihoods on the left hand side and a plot of TPR vs Precision on the right hand side.}
...
#'          TP = number of true positives, FN = number of false negatives, FP = number of false positives from the leave-one-out analysis.\itemize{ 
...
plot_loo_tpr_prec <- function(sa_input, rates = NULL, group_colors, by_group = TRUE, min_tpr = 0.80, min_pre = 0.95, min_thres = 0.50, thres_levels = seq(0.01, 0.99, by = 0.01), plots = c("facet", "paired", "both")[3], ncores = parallel::detectCores()){
  ...
  if(plots %in% c("facet", "both")){
    ...
    plot1 <- plotly::ggplotly(pre_dat %>% 
      dplyr::mutate(repunit = factor(repunit, levels = repunits)) %>% 
      ggplot2::ggplot(ggplot2::aes(x = pre, y = tpr, label = threshold, color = repunit)) + 
      ggplot2::geom_path() + 
      ...
      ggplot2::geom_vline(xintercept = min_pre, color = "red", linetype = 2)+
      ggplot2::geom_hline(yintercept = min_tpr, color = "red", linetype = 2)+
  ...
  if(plots %in% c("paired", "both")){
    ...
    plot2 <- lapply(repunits, function(repu){ 
      ...
      p2 <- plotly::ggplotly(pre_dat %>% 
                               ...
                               ggplot2::geom_point(data = point_dat %>% dplyr::filter(repunit == repu), ggplot2::aes( x = pre, y = tpr), color = "red")+
