...
#'
#' @description 
#' This function plots the results from [GCLr::calc_freq_pop()]. 
#' It creates a ("*.pdf") of bubble plots of allele frequencies for each locus. 
#' 
...
#'
#' @details
#' Allele frequency bubble plots are most useful for loci with multiple alleles (e.g. uSATs, microhaplotypes, combined loci). 
...
#' 
#' freqs <- GCLr::calc_freq_pop(sillyvec = sillyvec, loci = c("One_CO1.One_Cytb_17.One_Cytb_26", "One_MHC2_190.One_MHC2_251"), LocusCtl = GCLr::ex_LocusControl)
#' 
#' GCLr::plot_allele_freq_bubble(freq = freqs, file = path.expand("~/example_allele_freq.pdf"))
#' 
#'  
#' @export
plot_allele_freq_bubble <- function(freq, file, sillyvec = NULL, groupvec = NULL, loci = NULL, group_col = NULL, popnames = NULL, xlab.cex = 6, ylab.cex = 12, xtick.int = 1){
  ...
  if (is.null(groupvec)) {
    ...
    groupvec <- seq(length(freq$silly %>% 
                            unique()))
  ...
  # Making it easier to read numbers on the x-axis of plots
  ...
  freq_df <- freq %>% 
    dplyr::left_join(dplyr::tibble(silly = sillyvec, groupvec), by = "silly") %>% 
