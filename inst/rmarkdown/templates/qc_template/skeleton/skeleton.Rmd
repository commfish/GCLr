...
If the project type is TaqMan or GT-seq and the project species is Chinook, the following chunk checks to see if microsatellites were run on the project and drops them from the SILLY.gcl objects and LocusControl. Otherwise, the chunk outputs "Not applicable"
```{r remove_uSATs}

Ots_uSATs <- c("Ogo2v1", "Ogo4v1", "Oki100v1", "Omm1080v1", "Ots201bv1", "Ots208bv1", 
"Ots211v1", "Ots212v2", "Ots213v1", "Ots3Mv1", "Ots9v1", "OtsG474v1", 
"Ssa408uosv1") #These are the 13 GAPS uSATs

n_uSATs <- match(Ots_uSATs, loci) %>% 
  length()

if(species == "chinook" & any(project_type %in% c("TaqMan", "GT-seq")) & n_uSATs > 0){
  
  # Loop through the project sillys and drop microsat loci
  for(silly in paste0(project_sillys, ".gcl")){
    
    mod <- get(silly) %>% 
      dplyr::select(-tidyr::all_of(Ots_uSATs))
    
    assign(x = silly, value = mod)
    
  }
  
  loci <- loci[-match(Ots_uSATs, loci)]
...
if (any(species %in% c("chum", "sockeye")) & any(loci %in% alt_markers) & any(loci %in% failed_markers)) {
  ...
  alt_indivs <- tibble::tibble(silly_fish = character(0), alternate = logical(0))
...
Calculate individual heterozygosities and modified z-scores as a proxy for contamination.
...
(proj_het <- GCLr::calc_ind_het(sillyvec = project_sillys) %>% 
   dplyr::group_by(silly) %>%  # cutoffs are specific to each silly!
   dplyr::mutate(
...
Plot distribution of heterozygosity by project silly, noting outliers that are likely contaminated.
```{r warning=FALSE}
proj_het %>% 
  ggplot2::ggplot(ggplot2::aes(x = het, fill = outlier)) +
  ggplot2::geom_histogram(binwidth = 5 / length(LocusControl$locusnames)) +
  ggplot2::facet_wrap(~silly) +
  ggplot2::xlim(0, 1) +
...
### Genotyping rates plots

Check for missing samples in the tissue table.  
```{r tissued_data}

tissue_data <- GCLr::get_tissue_data(sillyvec = project_sillys, username = username, password = .password, import.vars = FALSE)

missing_tissue <- tissue_data %>% 
  dplyr::filter(IS_MISSING_PAIRED_DATA_EXISTS == "j") %>% 
  dplyr::select(SILLY_CODE, FK_FISH_ID) %>% 
  dplyr::mutate(SILLY_ID = paste0(SILLY_CODE, "_", FK_FISH_ID)) %>% 
  dplyr::pull(SILLY_ID)

if(length(missing_tissue) == 0){message("There are no missing samples in the project sillys")}

```

Create genotyping success rate object for plotting. Missing, contaminated, and wrong species samples will be given a 0% genotyping success rate in the interactive genotyping rate plots.
```{r genotyping_rates}

proj_geno <- lapply(project_sillys, function(silly){
  
  get(paste0(silly, "_raw.gcl"))
...
contam <- proj_het %>% 
  ...
  wrong_spp <- alt_indivs %>% 
  dplyr::filter(alternate == TRUE) %>% 
  dplyr::pull(silly_fish)

make_zero <- c(wrong_spp, contam, missing_tissue) %>% 
  unique()

  
proj_geno_rates0 <- proj_geno %>%  
  dplyr::select(COLLECTION_ID, SILLY_CODE, FK_FISH_ID, PLATE_ID, all_of(loci)) %>% 
  tidyr::pivot_longer(dplyr::all_of(loci), names_to = "locus", values_to = "geno") %>% 
  dplyr::group_by(COLLECTION_ID, SILLY_CODE, FK_FISH_ID, PLATE_ID) %>% 
  tidyr::replace_na(list(geno = "0")) %>% 
  dplyr::mutate(`Count of 0` = stringr::str_count(geno,"0") %>% sum()) %>% 
  tidyr::pivot_wider(names_from = locus, values_from = geno) %>% 
  dplyr::ungroup() %>% 
  dplyr::mutate(success_rate = 1-(`Count of 0`/length(loci)), SILLY_ID = paste(SILLY_CODE, FK_FISH_ID, sep = "_")) %>% 
  dplyr::select(SILLY_CODE, FK_FISH_ID, SILLY_ID, PLATE_ID, `Count of 0`, success_rate) 


if(length(make_zero) > 0){
  
  proj_geno_rates <- proj_geno_rates0 %>% 
    dplyr::mutate(success_rate = dplyr::case_when(SILLY_ID %in% make_zero ~ 0,
                                                  TRUE~success_rate)) %>% 
    dplyr::select(SILLY_ID, PLATE_ID, success_rate)
...
```
Get extraction data and join with proj_geno_rates
```{r extraction_info}

plate_ids <- proj_geno_rates$PLATE_ID %>% unique()

extraction_info <- GCLr::get_extraction_info(plate_ids = plate_ids, username = username, password = .password) %>% 
  dplyr::mutate(SILLY_ID = paste0(SILLY_CODE, "_", FISH_NO)) %>% 
  dplyr::select(WELL_NO, PLATE_ID = FK_PLATE_ID, SILLY_ID)

rm(.password) #ditch password since no longer needed

# This object contains adjusted success rate for each extraction plate in the project.
plot_data <- proj_geno_rates %>% 
  dplyr::left_join(extraction_info, by = c("SILLY_ID", "PLATE_ID")) %>% 
  tidyr::separate(SILLY_ID, into = c("silly", "sample"), sep = "_") %>% 
  dplyr::rename(plate = PLATE_ID, well = WELL_NO, GenotypeRate = success_rate)

```
This is a function is for saving plate heatmaps as an interactive html file.
```{r save_tags}
...
These plots can be used to help select high-quality samples for the QC analysis
```{r genotype_rate_plots}
if(!require("pacman")) install.packages("pacman");library(pacman) # install and load pacman
p_load(htmltools, htmlwidgets, tools) # load or install+load required packages.

#create platemap for plotting. Make the standard QC wells stand out with bigger circles and a red outline.
platemap_genotypeRate_singleSNP <- plot_data %>% 
  tidyr::separate(col = well, 
           into = c("Row", "Column"), 
           sep = "(?<=[A-Z])(?=[0-9])", #split between upper case letter and number
           convert = TRUE,
           remove = FALSE) %>% 
  dplyr::mutate(Row = factor(Row, levels = paste(rev(LETTERS)))) %>% 
  dplyr::mutate(size = dplyr::case_when(well %in% c("A3", "B4", "C5", "D6", "E7", "F8", "G9", "H10")~ 30,
                                 TRUE~20),
                outline = dplyr::case_when(well %in% c("A3", "B4", "C5", "D6", "E7", "F8", "G9", "H10")~ "#FF0000",
                                 TRUE~"#2c7bb6"))

#list of unique plates in project
plate_list <- sort(plate_ids)
...
plots <- lapply(seq_along(plate_list), function(i) {
  subset(platemap_genotypeRate_singleSNP, platemap_genotypeRate_singleSNP$plate==plate_list[i]) %>% 
    plotly::plot_ly(type = "scatter", # type of plot 
            ...
            x = ~Column, # x variable
            y = ~Row, # y variable
            marker = list(size =~size, # size of points, normal QC wells will have bigger circles
                          line = list(color = ~outline),
...
save_tags(plots, file = paste0(dirqc, "/", project, "_FilteredGenotypeRatePlots.html"), selfcontained = TRUE)

message(paste0("Extraction plate genotyping rate heatmaps have been save to: ", dirqc, "/", project, "_FilteredGenotypeRatePlots.html" ))

```


Now that a project has been vetted and you're happy with it, you can select high-quality fish for QC analysis.
...
```{r qc_genos}
qc_files <-
  list.files(path = paste0(dirqc, "/Genotype Data Files"), pattern = ".csv", full.names = TRUE, recursive = FALSE)
...
```{r concordance_files}
qc_concordance_report_file <- list.files(path = paste0(dirqc, "/Conflict Reports"), pattern = "Concordance", full.names = TRUE)
...
conflicts_by_plate <- combined_conflicts %>% 
  dplyr::group_by(plate_id, concordance_type, fish_id) %>%
...
Calculate individual heterozygosity and modified z-scores as a proxy for contamination.

*Note that for QC purposes we are not removing any fish, just noting how many per silly. We need to retain all fish, as they may be helpful in resolving conflicts later.*
```{r}
(qc_het <- GCLr::calc_ind_het(sillyvec = qc_sillys) %>% 
   dplyr::group_by(silly) %>%  # cutoffs are specific to each silly!
   dplyr::mutate(
     modified_z_score = 0.6745 * (het - median(het)) / mad(het),
     outlier = dplyr::case_when(abs(modified_z_score) > 3.5 ~ TRUE, TRUE ~ FALSE)
...
*Note - removing samples flagged as contaminated (heterozygosity outliers) in either project or QC.*
```{r dup_check_no_contamination_view}
if(!any("x" %in% names(dup_check_results))){
  
  dup_check_results %>%
    dplyr::filter(!(Keysillyvial %in% (qc_het %>% dplyr::filter(outlier) %>% dplyr::pull(SillySource)))) %>%  # remove contaminated QC fish
    dplyr::filter(!(Betweensillyvial %in% (proj_het %>% dplyr::filter(outlier) %>% dplyr::pull(SillySource)))) %>%  # remove contaminated project fish
    qc_table()
  
}else{dup_check_results}
```

## QC Power

Do we still have sufficient QC power after discounting missing loci and contaminated individuals in the project and QC? 

How many legit project + QC fish per DNA plate?
```{r}
# pool all QC
GCLr::pool_collections(collections = qc_sillys,
                       loci = loci,
                       newname = "qc_all") 

# remove contaminated QC fish
qc_all.gcl <- qc_all.gcl %>% 
  dplyr::filter(!(SillySource %in% (qc_het %>% dplyr::filter(outlier) %>% dplyr::pull(SillySource))))

# pool all project
GCLr::pool_collections(collections = project_sillys,
                       loci = loci,
                       newname = "project_all")

# remove contaminated project fish
project_all.gcl <- project_all.gcl %>% 
  dplyr::filter(!(SillySource %in% (proj_het %>% dplyr::filter(outlier) %>% dplyr::pull(SillySource))))

# count of legit project AND QC fish per PLATE_ID
qc_all.gcl %>% 
  dplyr::mutate(SillySource = stringr::str_remove_all(string = SillySource, pattern = "qc")) %>%  # remove qc from SillySource for join
  dplyr::select(SillySource) %>% 
  dplyr::inner_join(y = project_all.gcl %>% dplyr::select(SillySource, PLATE_ID), by = "SillySource") %>%  # inner join surviving project + QC fish
  tidyr::separate(col = SillySource, into = c("silly", "fish_id"), sep = "_") %>% 
  dplyr::group_by(PLATE_ID) %>% 
  dplyr::summarise(n = dplyr::n(),  # how many fish per plate?
                   FISH_IDs = paste(c(fish_id), collapse = ", ")) %>%  # list all fish IDs
  dplyr::filter(n < 8) %>%  # only show plates w/o all 8 QC fish
...
## QC Bounce Excel Workbook

Create QC bounce excel file and add alternate species individuals to individual conflict table if they exist.
...
GCLr::qc_bounce(QC_directory = dirqc, project = project, username = username, password = .password) 

rm(.password) #ditch password since no longer needed

if(any(species %in% c("chum", "sockeye")) & any(loci %in% alt_markers) & any(loci %in% failed_markers) & nrow(alt_indivs) > 0){
  
  bounce_file <-  paste0(dirqc, "/", project, "QC_bounce.xlsx") # Bounce filename
  
  ind_conflicts0 <- readxl::read_xlsx(path = bounce_file, sheet = "Conflicts by individual", col_types = c("text", rep("numeric", 3))) # Get individual conflicts, set col_types if no conflicts

  ind_conflicts <- dplyr::left_join(ind_conflicts0, alt_indivs %>% 
                                      dplyr::filter(alternate == TRUE), by = c("SillySource" = "silly_fish")
                                    ) %>% 
    tidyr::replace_na(replace = list(failure = 0, alternate = 0, non_missing_alt = 0)) # Join individual conflicts with alternate species individuals
  
  wb <- xlsx::loadWorkbook(file = bounce_file) # Load QC bounce workbook
  
  xlsx::removeSheet(wb, sheetName = "Conflicts by individual") # Remove existing sheet
  
  mySheet <- xlsx::createSheet(wb, sheetName = "Conflicts by individual") # Create new sheet
  
  xlsx::addDataFrame(ind_conflicts, mySheet, startRow=nrow(ind_conflicts)+2) # Add sheet to workbook
  
  xlsx::saveWorkbook(wb, file = bounce_file) # Save workbook
