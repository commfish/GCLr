...
if(species == "chinook" & any(project_type %in% c("TaqMan", "GT-seq"))){
  
  #Get the microsat loci
  uSatloci <- c(loci[grepl("v1", loci)], paste0(loci[grepl("v1", loci)], ".1"))
  
  # Loop through the project sillys and drop microsat loci
  for(silly in paste0(project_sillys, ".gcl")){
    
    mod <- get(silly) %>% 
      dplyr::select(-tidyr::all_of(uSatloci))
    
    assign(x = silly, value = mod)
    
  }
  
  loci <- loci[!grepl("v1", loci)]
